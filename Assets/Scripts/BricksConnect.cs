...
public class BricksConnect : MonoBehaviour
{
    ...
    void AddGraph(List<GameObject> FemaleBricksList, List<GameObject> MaleBricksList, List<GameObject> StructureList,
        ...
        if (StructureList.Count == 0){
            ...
            Dictionary<string, List<GameObject>[]> Graph = ConnectStructure.GetComponent<ConnectStructure>().Graph;

            foreach (GameObject female in FemaleBricksList){
                Graph.Add(female.name, ArrayOfList(null, MaleBricksList));
                female.GetComponent<BrickCollisionHandler>().ConnectStructure = ConnectStructure;
            } 
            foreach (GameObject male in MaleBricksList){
                Graph.Add(male.name, ArrayOfList(FemaleBricksList, null));
        ...
        //need to modify to not compare grabbed, but the outer one from GetStructureList function
            ...
            if (grabbedBrick == femaleBrick){
                Dictionary<string, List<GameObject>[]> Graph 
                ...
                foreach (GameObject brick in MaleBricksList){
                    if(Graph[brick.name][0][0] == null){
                        Graph[brick.name][0].Remove(null);
                    }
                    Graph[brick.name][0].Add(femaleBrick);
                }
                Graph.Add(femaleBrick.name, ArrayOfList(null, MaleBricksList));
                ...
                foreach (GameObject brick in FemaleBricksList){
                    if(Graph[brick.name][1][0] == null){
                        Graph[brick.name][1].Remove(null);
                    }
                    Graph[brick.name][1].Add(maleBrick);
                }
                ...
                Graph.Add(maleBrick.name, ArrayOfList(FemaleBricksList, null));
            }
